...
-- Custom actions picker (like <leader>sk but for personal tasks)
vim.keymap.set("n", "?", function()
  local actions = require("config.custom_actions")
  Snacks.picker.pick({
    title = "Custom Actions",
    items = vim.tbl_map(function(a)
      return { text = a.desc, action = a.action, preview_lines = a.preview }
    end, actions),
    format = function(item)
      return { { item.text } }
    end,
    preview = function(ctx)
      local lines = ctx.item.preview_lines or { ctx.item.text }
...
end, { noremap = true, silent = true, desc = "Custom Actions" })
